...
export class UserProgressService {
	...
	updateStreak(): void {
		...
		const yesterday = this.dateService.getYesterdayString();

		let newStreak = current.streak;

		if (current.lastWorkoutDate === yesterday) {
			newStreak += 1;
		} else if (current.lastWorkoutDate !== today) {
			newStreak = 1;
		}
	...
	recordDifficulty(difficulty: 'EASY' | 'NORMAL' | 'HARD'): boolean {
		const current = this.progress();
